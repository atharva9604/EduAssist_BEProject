...
export async function uploadTimetableFull(file: File): Promise<{ success: boolean; inserted: number; total_events: number }> {
  const form = new FormData();
  form.append('file', file);
  // Full sync for all weekdays to upcoming dates
  const res = await fetch(`${API_BASE}/api/upload-timetable?mode=replace`, { method: 'POST', body: form });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}