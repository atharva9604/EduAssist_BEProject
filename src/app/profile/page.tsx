...
import { Plus, Trash2, BookOpen, Award, Presentation, FileCheck, Briefcase, FileText, Download, X, Users, Search } from "lucide-react";
import ProfileAddFormModal from "@/components/ProfileAddFormModal";
import AttendanceManager from "@/components/AttendanceManager";
import ResearchLookoutModal, { type LookoutData } from "@/components/ResearchLookoutModal";
...
const ProfilePage = () => {
  ...
  const [lookoutData, setLookoutData] = useState<LookoutData | null>(null);
  ...
  const handleResearchLookout = async (title: string, description: string) => {
    setShowLookout(true);
    ...
    try {
      ...
      if (json.success) {
        setLookoutData(json.lookoutData);
      }
    } catch (e) {
      console.error("Research Lookout failed:", e);
    } finally {
            ...
            <div className="bg-gray-900 rounded-xl p-8 border border-gray-700 shadow-lg">
              ...
              <div className="grid gap-4">
                {projects.length === 0 ? (
                  ...
                  projects.map((item) => (
                    <div key={item.id} className="bg-gray-800 p-5 rounded-xl border border-gray-700 hover:border-gray-600 transition group">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleResearchLookout(item.title, item.description || "")}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-[#DAA520]/15 text-[#DAA520] border border-[#DAA520]/40 rounded-lg hover:bg-[#DAA520]/25 hover:border-[#DAA520] transition"
                            title="Research Lookout"
                          >
